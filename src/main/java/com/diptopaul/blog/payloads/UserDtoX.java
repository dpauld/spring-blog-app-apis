...
public class UserDtoX {
	...
	@Size(min=4, message = "name must be min of 4 charachters!!", groups = {UpdateValidation.class})
	@NotEmpty(message = "name required",groups = {UpdateValidation.class})
	private String name;
	...
	@Size(min=5, message = "password must be 5 charachters long.", groups = {PasswordResetValidation.class})
	@Size(max=15, message = "password can not exceed 15 charachter.", groups = {PasswordResetValidation.class})
	@Pattern(regexp = ".*[0-9].*", message = "password must contain one digit", groups = {PasswordResetValidation.class})
	@NotEmpty
	//order of the annotation matters, the lower the order, it will be checked first
